...
class LLMRobotServer:
    def __init__(self, args):
        ...
        # ASR relative
        self.asr_model = WhisperForConditionalGeneration.from_pretrained(args.asr_model)
        self.asr_processor = WhisperProcessor.from_pretrained(args.asr_model)
        self.asr_dict = {}
    ...
    def compose_robot_input(
        ...
        if isinstance(instruction, list) and len(instruction) == 2:
            # This is audio mode but we use a ASR model to get the instruction
            spk_id = instruction[1]
            audio_id = self.eval_instruct2id[instruction[0]]
            eval_audio_path = os.path.join(
                self.eval_audio, spk_id, f"{audio_id:04}" + ".wav"
            )
            if eval_audio_path in self.asr_dict:
                transcription = self.asr_dict[eval_audio_path]
            else:
                eval_audio = librosa.load(eval_audio_path, sr=16000)[0]
                audio_tensor = self.asr_processor(
                    eval_audio, sampling_rate=16000, return_tensors="pt"
                ).input_features
                predicted_ids = self.asr_model.generate(audio_tensor)
                transcription = self.asr_processor.batch_decode(predicted_ids, skip_special_tokens=True)[0]
                self.asr_dict[eval_audio_path] = transcription
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        default="/zhaowei/workspace/LLaVA/checkpoints/llava-v1.5-7b-calvin-rel-obs-reduce5/checkpoint-10786",
    )
    ...
    parser.add_argument(
        "--eval_instructions",
        type=str,
        default="/zhaowei/workspace/LLaVA/playground/calvin_data_audio/eval/new_playtable_validation.yaml",
    )
    parser.add_argument(
        "--eval_audio", type=str, default="/zhaowei/data/calvin_eval_audio"
    )
    parser.add_argument("--asr_model", type=str, default="/zhaowei/models/models--openai--whisper-large-v2/snapshots/ae4642769ce2ad8fc292556ccea8e901f1530655")
    ...
    def find_audio_files(directory):
        ...
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith(('.mp3', '.wav', '.flac', '.aac', '.ogg', '.wma')):
                    audio_files.append(os.path.join(root, file))
        return audio_files

    audios = find_audio_files("/zhaowei/data/calvin_eval_audio/1089")

    for eval_audio in audios:
            eval_audio = librosa.load(eval_audio, sr=16000)[0]
            audio_tensor = llm_robot.asr_processor(
                eval_audio, sampling_rate=16000, return_tensors="pt"
            ).input_features
            predicted_ids = llm_robot.asr_model.generate(audio_tensor)
            transcription = llm_robot.asr_processor.batch_decode(predicted_ids, skip_special_tokens=True)[0]
            print(transcription)
