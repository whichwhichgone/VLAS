...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio = librosa.load(os.path.join(self.audio_folder, audio_file), sr=16000)[0]
        ...
        return input_ids, image_tensor, audio_tensor, image.size
...
def collate_fn(batch):
    input_ids, image_tensors, audio_tensors, image_sizes = zip(*batch)
    input_ids = torch.stack(input_ids, dim=0)
    image_tensors = torch.stack(image_tensors, dim=0)
    audio_tensors = torch.stack(audio_tensors, dim=0)
    return input_ids, image_tensors, audio_tensors, image_sizes
...
def eval_model(args):
    ...
    raw_questions = [
        json.loads(q) for q in open(os.path.expanduser(args.raw_question_file), "r")
    ]
    raw_questions = get_chunk(raw_questions, args.num_chunks, args.chunk_idx)
    ...
    for (input_ids, image_tensor, audio_tensor, image_sizes), line, raw_line in tqdm(
        zip(data_loader, questions, raw_questions), total=len(questions)
    ):
        idx = line["question_id"]
        cur_prompt = raw_line["text"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--raw-question-file", type=str, default="")
