...
def preprocess_multimodal_asr(
    sources: Sequence[str],
    data_args: DataArguments
) -> Dict:
    is_multimodal = data_args.is_multimodal
    is_multimodal_audio = data_args.is_multimodal_audio
    if not is_multimodal and not is_multimodal_audio:
        return sources
    ...
    if is_multimodal_audio:
        # audio data is clean already, which is '<audio>\n'
        pass
    return sources
...
def preprocess_v1_asr(
    ...
    # Tokenize conversations
    if has_image and not has_audio and not has_goal:
        input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    elif has_audio and not has_image and not has_goal:
        input_ids = torch.stack([tokenizer_audio_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    elif not has_audio and not has_image and not has_goal:
        input_ids = tokenizer(
            conversations,
            return_tensors="pt",
            padding="longest",
...
def preprocess_plain_asr(
    ...
    for source in sources:
        ...
        assert not (DEFAULT_IMAGE_TOKEN in source[0]['value'] and DEFAULT_AUDIO_TOKEN in source[0]['value'])
    ...
    targets = copy.deepcopy(input_ids)
    for target, source in zip(targets, sources):
        tokenized_len = len(tokenizer_audio_token(source[0]['value'], tokenizer))
        target[:tokenized_len] = IGNORE_INDEX
...
def preprocess_asr(
    ...
    """
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.PLAIN:
        return preprocess_plain_asr(sources, tokenizer)
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1_asr(sources, tokenizer, action_tokenizer, has_image=has_image, has_goal=has_goal,has_audio=has_audio, has_embody=has_embody)
    if conversation_lib.default_conversation.version == "mpt":
        return preprocess_mpt(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
...
class LazySupervisedDataset(Dataset):
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            audio_tokens = 512 if 'audio' in sample else 0
            length_list.append(sum(len(conv['value'].split()) for conv in sample['conversations']) + img_tokens + audio_tokens)
        return length_list
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            if 'image' in sample and 'audio' in sample:
                cur_len = cur_len + 1024
            elif 'image' in sample and not 'audio' in sample:
                cur_len = cur_len + 128
            elif not 'image' in sample and 'audio' in sample:
                cur_len = cur_len + 512
            else:
                cur_len = -cur_len
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # NOTE: 1. 如果存在goal img，则对会话数据中的图像数据进行初步预处理, 形成(3, 336, 336)的张量
        if 'goal' in sources[0]:
            goal_file = self.list_data_dict[i]['goal']
            goal_folder = self.data_args.image_folder
            processor = self.data_args.image_processor
            goal = Image.open(os.path.join(goal_folder, goal_file)).convert('RGB')
            if self.data_args.image_aspect_ratio == 'pad':
                def expand2square(pil_img, background_color):
                    width, height = pil_img.size
                    if width == height:
                        ...
                        result.paste(pil_img, ((height - width) // 2, 0))
                        return result
                goal = expand2square(goal, tuple(int(x*255) for x in processor.image_mean))
                goal = processor.preprocess(goal, return_tensors='pt')['pixel_values'][0]
            else:
                goal = processor.preprocess(goal, return_tensors='pt')['pixel_values'][0]
        
        # NOTE: 2. 如果存在音频，则对会话数据中的音频数据进行初步预处理, 形成(80, 3000)的张量
        if 'audio' in sources[0]:
            audio_file = self.list_data_dict[i]['audio']
            if 'image' in sources[0]:
                audio_folder = self.data_args.audio_folder
            else:
                audio_folder = self.data_args.audio_folder_asr
            audio_processor = self.data_args.audio_processor
            audio, sr = librosa.load(os.path.join(audio_folder, audio_file), sr=16000)
        ...
        # NOTE: 3. 如果存在图像或者音频，则对多模态数据的会话形式统一进行处理，否则只提取其原始会话
        ...
        data_dict = preprocess_asr(
            sources,
            self.tokenizer,
            self.action_tokenizer,
            has_image=('image' in self.list_data_dict[i]),
            has_goal=('goal' in self.list_data_dict[i]),
        ...
        elif self.data_args.is_multimodal_audio:
            mel_bins = self.data_args.audio_processor.feature_extractor.feature_size
            num_frames = self.data_args.audio_processor.feature_extractor.nb_max_frames
            data_dict['audio'] = torch.zeros(mel_bins, num_frames)
        return data_dict
