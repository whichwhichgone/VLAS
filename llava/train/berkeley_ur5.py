...
from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_AUDIO_TOKEN
...
from llava.action_tokenizer import ActionTokenizer, encode_actions_real, encode_robot_obs
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if '<image>' in rou and not '<audio>' in rou:
                round_len = len(tokenizer_image_token(rou, tokenizer))                   # 多了<s>, 但少了</s>, 所以不变
                instruction_len = len(tokenizer_image_token(parts[0], tokenizer)) - 2    # 多了<s>, 且': '在句尾时多了' ', 所以减2
            ...
            elif '<image>' in rou and '<audio>' in rou:
...
def preprocess_asr(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1_asr(sources, tokenizer, action_tokenizer, has_image=has_image, has_audio=has_audio, has_embody=has_embody)
    if conversation_lib.default_conversation.version == "mpt":
        return preprocess_mpt(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'audio' in sources[0]:
            ...
            audio_folder = self.data_args.audio_folder if 'image' in sources[0] else '/zhaowei/data/LibriSpeech/train-clean-100'
...
def train(attn_implementation=None):
    ...
    action_to_lang = partial(encode_actions_real, statistics=data_args.action_stat)
    ...
    # NOTE: Tower for audio processing
